...
proc quote:cmd:quote {0 1 2 3 {4 ""}  {5 ""}} {
	...
	# -- add last line spoken
		...
		foreach key $sorted {
			...
			debug 3 "\002quote:\002 line: $lastspeak($tchan,$tts,[quote:escape $atnick])"
			if {[string tolower $chan] ne [string tolower $tchan]} { continue; }
			putlog "atnick: $atnick -- tnicks: $tnicks"
			if {[string tolower [join $atnick]] in [string tolower $tnicks] || $tnick eq "*"} {
				...
				debug 3 "\002quote:\002 appending lquote: $lastspeak($tchan,$tts,[quote:escape $atnick]) -- count: $count"
			...
			if {$count eq $lines} {
				...
				foreach tsnick $ascending {
					if {$newline} { set xtra "\\n" } else { set xtra "" }
					append tquote "  $lastspeak($tchan,[quote:escape $tsnick])$xtra"
				}
				break;
			}
		}
...
proc quote:addspeak {nick uhost hand chan text} {
	...
	set lastspeak($chan,$ts,$nick) $text
	timer [cfg:get lastspeak:mins $chan] "arm::debug 5 \"quote:pubm: unsetting lastspeak($chan,$ts,[quote:escape $nick])\"; unset arm::lastspeak($chan,$ts,[quote:escape $nick])"
}
